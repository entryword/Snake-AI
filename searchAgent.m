...
switch evluFuc
    ...
    case 3
        ...
        for i = 1 : length(gameState.snake)
            if i==idx
                for j = 2:length(gameState.snake(idx).pos)
                    x = min(abs(gameState.snake(idx).pos(1,2)-gameState.snake(idx).pos(j,2))...
                        ,gameState.size(1)-abs(gameState.snake(idx).pos(1,2)-gameState.snake(idx).pos(j,2)));
                    y = min(abs(gameState.snake(idx).pos(1,1)-gameState.snake(idx).pos(j,1))...
                        ,gameState.size(2)-abs(gameState.snake(idx).pos(1,1)-gameState.snake(idx).pos(j,1)));
                    dist2 = x+y;
                    if dist2<5
                        value = value+mySnakeVal/dist2;
                    end
                ...
                for j = 1 : length(gameState.snake(i).pos)
                    x = min(abs(gameState.snake(idx).pos(1,2)-gameState.snake(i).pos(j,2))...
                        ,gameState.size(1)-abs(gameState.snake(idx).pos(1,2)-gameState.snake(i).pos(j,2)));
                    y = min(abs(gameState.snake(idx).pos(1,1)-gameState.snake(i).pos(j,1))...
                        ,gameState.size(2)-abs(gameState.snake(idx).pos(1,1)-gameState.snake(i).pos(j,1)));
                    dist2 = x+y;
                    if dist2<5
                        value = value+otherSnakeVal/dist2;
                    end
